...
const studentList=students.map((s,i) =>`${i+1}. ${s.firstName}`)
document.querySelector("#task2 p").innerHTML=studentList.join(", ")
...
//task 4
document.querySelector("#task4 p").innerHTML=numbers.filter(n=>(n>40)).length

//task 5
document.querySelector("#task5 p").innerHTML=numbers.filter(n=>(n>=20&&n<=50))
...
document.querySelector("#task6 input").addEventListener("input",ev=>{
    ...
    document.querySelector("#task6 p").innerHTML=len
})